...
| 4 | – | Eroj in der Breite (Formular, Navigation, Feedback, Daten), Sxablonoj. **Regulo-Kandidat (Befund der Abnahme von Spec 004):** eigene Kontrastkategorie für Beschriftungstext — Buttonbeschriftungen und Statustexte sind kurz und kräftig gesetzt, APCA sieht dafür ein niedrigeres Band vor als für Fließtext; einzuführen mit einem eigenen beratenden Band, wenn mehr Eroj die Rollen belegen. Die WCAG-Schwellen bleiben unberührt. **Anforderung aus Spec 003 F8 (Jugxo `jug_01M307X4DQQWRFTDXB1S5CGPN9`):** Sobald eine zweite Aspekto auf einer gebundenen Rolle eine andere Transparenz hat als die erste, trägt die Figma-Projektion diese Rolle nicht mehr — eine Fläche hat eine Deckkraft, die Modusumschaltung verlangt zwei. Dann bekommt jeder gebundene Teil einen Auflagen-Knoten, dessen `opacity` an eine FLOAT-Variable gebunden ist; das Alpha läuft durch dieselbe Kaskade versteckter Helfer wie die Farben. Heute betroffen wären genau vier Rollen (`color.action.tertiary.rest|hover|pressed|disabled`, je `surface.fill` und `border.color`) | geplant |
