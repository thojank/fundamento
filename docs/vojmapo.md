...
| 3b | [004](../specs/004-komparo/) | **Komparo:** komuna 2 als starkes Basis-Theme mit messbaren Zielen, neues Celo **Vitrino** (visuelle Übersicht, Laufzeit-Umschaltung, Gegenüberstellung), Adobe Spectrum als Benchmark-Aspekto `komparo` im getrennten Repo (erste Stufe Enportilo), Bericht ohne Gesamtscore, Amendment Art. V v1.7 | 🚧 Etappe A umgesetzt, Abnahme offen (M1 an der laufenden Vitrino; Constitution v1.7 in dieser Phase); Etappe B (Import, Abbildung, Bericht) folgt im getrennten Repo `fundamento-aspekto-komparo` |
| 4 | – | Eroj in der Breite (Formular, Navigation, Feedback, Daten), Sxablonoj. **Regulo-Kandidat (Befund der Abnahme von Spec 004):** eigene Kontrastkategorie für Beschriftungstext — Buttonbeschriftungen und Statustexte sind kurz und kräftig gesetzt, APCA sieht dafür ein niedrigeres Band vor als für Fließtext; einzuführen mit einem eigenen beratenden Band, wenn mehr Eroj die Rollen belegen. Die WCAG-Schwellen bleiben unberührt. ~~**Anforderung aus Spec 003 F8 (Jugxo `jug_01M307X4DQQWRFTDXB1S5CGPN9`):** Sobald eine zweite Aspekto auf einer gebundenen Rolle eine andere Transparenz hat als die erste, trägt die Figma-Projektion diese Rolle nicht mehr — eine Fläche hat eine Deckkraft, die Modusumschaltung verlangt zwei. Dann bekommt jeder gebundene Teil einen Auflagen-Knoten, dessen `opacity` an eine FLOAT-Variable gebunden ist.~~ **Erledigt, die Annahme war falsch (F37, 2026-09-23, Jugxo `jug_01M37BYR772C5N3AF7J5451DRB`):** Figmas COLOR-Variablen führen den Alphakanal mit, und der aufgelöste Alphawert erscheint am Paint als `opacity` — gemessen an einem Knoten bei identischer Bindung in zwei Modi: komuna `#000000 α0`, ekzemplo/light `#ffffff α1`. Eine Fläche hat so viele Deckkräfte, wie die Sammlung Modi hat. Der Auflagen-Knoten wird nicht gebraucht; die falsche Annahme bleibt hier stehen, weil sie erklärt, warum die Projektion sich so lange geweigert hat | geplant |
...
| Figma zeichnet die Icons der Schaltfläche nicht | Paket „Figma zeigt das Ero" (Jugxo `jug_01M31TV1V4S3RNBPWSRB2CGEDK`) | Die Vitrino entscheidet, was Figma zeigt, und sie zeigt keine Icons. `check:parity` meldet `icon.color` und `icon.size` als benannte, freigegebene Differenz (`parity-part-not-drawn`, Warnung). Sobald die Vitrino ein Icon zeigt, entfällt der Eintrag in `FIGMA_NOT_DRAWN` und Figma zeichnet es |
