...
| `pnpm check:release` | The release path of the Make kits, as `release.yml` runs it: both kits built (komuna from the repository, ekzemplo from its fixture), each present and without a font file, each packed as a dry run (`scripts/release-kits.sh pack`). |
