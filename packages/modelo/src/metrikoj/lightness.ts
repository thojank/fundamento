...
export interface StepConsistency {
  /** Median distance of the inner steps; 0 when the ramp has none. */
  median: number;
 ...
 * median of the inner steps. The first and the last step are the ends of the ramp and stay out —
 * near white the eye needs finer steps, and at the dark end a half step is common; a jump in the
 * middle is what makes a ramp useless as a tool (Spec 004, D-03).
 */
export function oklchLStepConsistency(ramp: readonly RampStep[]): StepConsistency {
  ...
  const middle = median(inner.map((step) => step.delta));
  ...
  for (const step of inner) {
    const deviation = Math.abs(step.delta / middle - 1);
    if (deviation > worst) {
      worst = deviation;
