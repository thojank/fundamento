...
/** The single-file sources of the kit: the element inline, and the React wrapper beside it. */
function sources(input: CeloInput, aspekto: string): GeneratedFile[] {
  ...
  const inline = input.modelo.eroj
    .map((entry) => {
...
${elementSource({ ...namesOf(entry.ero), ero: entry.ero.name, inline: true })}`;
  ...
  const defines = input.modelo.eroj
    .map(
      (entry) =>
        `  if (customElements.get("${namesOf(entry.ero).tag}") === undefined) customElements.define("${namesOf(entry.ero).tag}", ${namesOf(entry.ero).className});`,
    )
    .join("\n");
  return [
    {
...
`,
    },
    {
      ...
      text: header + reactSource(input.modelo.eroj, "./index.js"),
    },
  ];
}
...
function declarations(input: CeloInput): string {
  const lines = input.modelo.eroj.flatMap((entry) => {
    const { className } = namesOf(entry.ero);
    const name = componentName(entry.ero);
    return [
...
${reactSource(input.modelo.eroj, "./index.js", { typesOnly: true })}
${lines.join("\n")}
