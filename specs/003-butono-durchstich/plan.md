...
- **Collections:** one per Dimensio (modes = its values, at most 3 in this Modelo), plus one single-mode collection `fundamento` for the tokens no Dimensio changes. **Plan limits (checked 2026-09-20, research §6.2):** Professional allows up to 10 modes per collection, Organization 20, Enterprise unlimited; Starter has one mode only. The import therefore runs fully **from Professional**, with room for up to 10 Aspektoj in one library; Starter cannot carry the Dimensio collections.
- **Empty collections are not created.** Because every Aspekto restates every core token (Art. IV completeness), the `aspekto` Dimensio changes every token and the collection `fundamento` stays empty in a Modelo with an external Aspekto. The plan still lists it, so the structure is stable; the plugin creates no collection without variables (review decision, Stage 2–4).
...
| **Ten Aspektoj per library on the Professional plan** (D-12) | Plan-independence; modes per collection are plan-limited (research §6.2, checked 2026-09-20). | One library per Aspekto beyond ten brands, or Organization (20) / Enterprise (unlimited). |
