...
- **Ausweichregel, drei Fassungen; 8 Ausweichstufen, nach der sunken-Korrektur 11:** (1) Die erste Fassung protokollierte jeden Einzelschritt als eigenen Eintrag (46). Dieselbe Rolle erschien dadurch bis zu elfmal; es war eine Mehrfachzählung, keine 46 Eingriffe. Zudem galten die light/high-Werte aus `+contrast/high` auch in dark/high und mussten dort zurückgedreht werden. (2) Mit dem Sprung zur ersten bestehenden Stufe und einem Eintrag je Klasse und Rolle blieben 11, davon 3 nur Reparaturen dieses Durchsickerns in dark/high. (3) Mit `+color-scheme/light+contrast/high` für light/high entfällt das Durchsickern: 8 echte Ausweichstufen (2 light/default, 1 dark/default, 4 light/high, 1 dark/high). (4) Die Korrektur `background.sunken` → `neutral.300` (unten) macht sunken dunkler; `text.muted` und `text.subtle` auf sunken kippen und wandern je eine Stufe weiter (light/default: muted 600 → 800 statt 700, subtle 700 → 800; light/high: beide 800 → 900). Endstand: 11 Ausweichstufen (4 light/default, 1 dark/default, 5 light/high, 1 dark/high). Nebenwirkung: subtle und muted fallen in light auf dieselbe Stufe; ihre Abstufung geht dort verloren. Ein Enportilo braucht diese Regel samt Protokoll von Anfang an.
- **Ergebnis prüfen (zweiter Enportilo-Prototyp):** `scripts/preview.mjs` rendert `preview/index.html` nur aus `rezolvoj.json` des Exports, ohne Framework und ohne Abhängigkeit. Es zeigt je Kombination (light/dark × default/high, viewport medium, density comfortable) eine Karte mit Flächen, Text-Rollen, Aktionen, Status, Fokusring und Rändern, jeweils mit WCAG-Kontrast, dazu die Typografie-Rollen und die Paletten-Rampen. Schon der erste Blick fand, was die Prüfungen nicht finden: `background.sunken` war in light heller als Canvas und Default, weil die Kernrolle `neutral.100` auf den Anker "dark ink" fiel und keine Markenabsicht sunken umhängte. Korrigiert (vom Maintainer entschieden): sunken → `neutral.300`, die Stufe unter Paper, als Markenabsicht im Skript.
- **Regulo-Kandidat für den Kern: `surface-order`** (vom Maintainer festgelegt; Umsetzung in Phase 2 mit den weiteren Reguloj, nicht in Phase 1). Der Befund wiegt mehr als die Korrektur: Vollständigkeit und Kontrast prüfen Werte, aber nicht die Ordnung der Flächen.
  - Aussage: Die relative Helligkeit der Flächenrollen folgt ihrer Rangfolge, im Light-Schema `sunken ≤ canvas ≤ default ≤ raised`, im Dark-Schema umgekehrt.
  ...
  - Offen für die Umsetzung: die Richtung im Dark-Schema. Die heutigen Daten steigen auch dort von sunken zu raised an; dunkle Flächen werden mit der Höhe heller (komuna dark: sunken und canvas `neutral.1000`, default `neutral.950`, raised `neutral.900`; ciferecigo dark ebenso). Eine wörtlich "umgekehrte" Ordnung würde beide Aspektoj als Bruch melden. Phase 2 muss vor der Umsetzung klären, ob die Regel im Dark-Schema dieselbe Richtung meint oder komuna geändert wird.
