...
- **Befund für den Kern: komuna vereint subtle und muted unter `contrast=high`.** `contrast/high` hängt beide auf `neutral.800`, `color-scheme/dark+contrast/high` beide auf `neutral.100`. Wörtlich genommen wäre "in keiner Kombination auf derselben Stufe" damit für jedes Aspekto unerfüllbar. Regel 6 gilt daher dort, wo der Kern die beiden trennt (light/default, dark/default); in light/high und dark/high bleibt ciferecigo wie der Kern. Ob der Kern unter hohem Kontrast die Hierarchie erhalten soll (z. B. muted 800, subtle 900), ist eine Entscheidung für Phase 2, zusammen mit `surface-order`.
...
- **Regulo-Kandidat für den Kern: Texthierarchie** (aus Regel 6 abgeleitet, zur Entscheidung in Phase 2): Die Textrollen default, subtle und muted liegen auf verschiedenen Stufen in der Reihenfolge ihres Rangs. Eine Kontrastprüfung erkennt ein Zusammenfallen nicht.
