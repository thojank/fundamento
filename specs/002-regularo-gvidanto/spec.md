...
- **FR-02 `text-hierarchy`** (automatic): In jeder Kombination sind die aufgelösten Werte von `color.text.default`, `.subtle` und `.muted` paarweise verschieden, und ihr Kontrast gegen `background.default` fällt in dieser Reihenfolge (default ≥ subtle ≥ muted). Kialo: Die Textrollen tragen eine Bedeutungsunterscheidung; hoher Kontrast darf sie nicht löschen.
