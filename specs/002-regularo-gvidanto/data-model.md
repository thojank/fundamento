...
| `state-distinct` | In every combination, the OKLCH lightness of `color.action.<v>.hover`, `.pressed` and `.selected` differs from `.rest` by at least 0.04, for every action variant. | A state that looks like the rest state gives no feedback, and a difference in hue alone disappears for people with a colour-vision deficiency; no contrast check sees either. | `tokens: [color.action.*.hover, color.action.*.pressed, color.action.*.selected, color.action.*.rest]` | `{ metric: oklch-l-delta, min: 0.04 }` | `state-distinct` |
...
| `contrast/high` | `color.text.default` | – (core `neutral.900`) | `{color.palette.neutral.950}` | D-05 |
| `contrast/high` | `color.text.subtle` | `{color.palette.neutral.800}` | `{color.palette.neutral.900}` | D-05 |
| `contrast/high` | `color.text.muted` | `{color.palette.neutral.800}` | unchanged | D-05 |
